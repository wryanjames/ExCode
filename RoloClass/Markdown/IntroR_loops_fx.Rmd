...
### `plyr` and `dplyr`
The `plyr` and `dplyr` are both packages that are useful for vector operations. The functions used are dependent of the type of data.
...
fun.summ <- function(x){
  ...
  data.frame(mean.count = mean.count, sd.count = sd.count, cv.count = cv)
  
  }

ddply(d, "year", fun.summ)
...
###
#plyr ddply Example 2
###
summ.2 <- ddply(d, "year", mutate, std.value = (count - mean(count))/sd(count), cum.sum = cumsum(std.value))
summ.2

#dplyr Example 2
summ.3 <- d %>% 
  group_by(., year) %>% 
  mutate(std.value = (count - mean(count))/sd(count), cum.sum = cumsum(std.value))%>% 
  data.frame()

summ.3
###
#plyr dlplyr Example 3
###
data("iris")

model.lm.spp <- plyr::dlply(iris, .(Species), function(x){lm(Sepal.Length ~ Petal.Length, data = x)})
model.lm.spp

#dplyr Example 3
model.lm.spp2 <- iris %>% 
  group_by(Species) %>% 
  do(fit.model = lm(Sepal.Length ~ Petal.Length, data = .))

# tidy(model.lm.spp2, fit.model)

###
#plyr ldply Example 4
###

extractfun <- function(m){
  r2 <-summary(m)$r.sq
  AIC <- AIC(m)
  data.frame(Rsq = r2, AIC = AIC)
}

output.lm.spp <- ldply(model.lm.spp, extractfun)
output.lm.spp
...
#A solution to save the results in a list, 
#and then combine into a single vector after the loop is done
means <- c(0, 1, 2)
...
## Homework
### R Homework – 1: Loops and density-independent and -dependent growth models
Please, using RStudio and R, answer the questions below. You can use the class notes, the recommended books, and other web materials to help you answer the questions. You can work on the homework in groups, but please independently send the R code and the answers to the short-essay questions. 

**In three weeks (10/19/2021)**, please submit your R code (.R file) and short answers (.docx file) to CANVAS using the following filename structure LastName_HW1_Filetype_Date (e.g., Santos_HW1_R_20210928). Again, you can share and work together on the code, but independently, in your own words, submit your answers to the short essay questions.  

For this homework, you will use a dataset that I will attach in the R workshop module in CANVAS. This dataset comprises the catch-per-unit effort (CPUE; a proxy of relative abundance) across three decades for three recreational fish species targeted in the Everglades National Park (catch.csv). Use the function read.csv file to import the catch.csv as data frame in RStudio. 

#### Question 1
a. Using R, calculate lambda from one year to the next (i.e., for each time step) for the three fish species using the CPUE as a proxy of abundance. 

b. Convert lambda (λ, finite rate of increase) to r (instantaneous rate of increse)

c. Convert r to a monthly rate and back-transformed it to lambda. Append the new values as separated columns in the catch data frame. 

#### Question 2
a. Calculate mean(λ) and sd(λ) for the three species using your calculation from Question 1. Try to use either a for-loop or functions in `plyr`, `dplyr` and `purr` to calculate mean(λ) and sd(λ).

b. Using ggplot, plot the mean(λ) and sd(λ) for the three species. Hint: `geom_point()`, `geom_errorbar()` or `geom_pointrange()`.

c. Short essay question 2.1: Based on the mean(λ) and sd(λ) for the three species, what can you tell me about the population growth and fluctuations that these species experienced? Based on the mean(λ), what will be the expected growth into the future? Why?

#### Question 3 
a. Calculate the mean, median, and the 25 and 95 percent quantiles of the species CPUE

b. Using a discrete density-dependent growth model, project the population growth for the three species to 50 years based on three carrying capacity values (K: mean(CPUE), median(CPUE), 95-quantile(CPUE)). Use the mean(λ), the mean(CPUE), median(CPUE) and 95-quantile(CPUE) as your K values to project the population growth. Hint: 1 – rd = λ; See Gotelli pages 35-37 and pages Stevens 62-68

c. Using ggplot, plot the population projections for each species. Hint: `facet_wrap()` or `facet_grid()`
